...
export function RulesPage() {
  ...
  const [ruleType, setRuleType] = useState<RuleType>('TEACHER_MONTHLY_HOURS_MIN')
  const [teacherId, setTeacherId] = useState<number | null>(null)
  const [groupId, setGroupId] = useState<number | null>(null)
  const [intValue, setIntValue] = useState('')

  const { data: annexes = [] } = useGetAnnexesQuery()
  const { data: rules = [], isLoading } = useGetAnnexRulesQuery(selectedAnnexId!, {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-2xl">
      ...
      {adding && (
        <div className="rounded-lg border border-border p-4 flex flex-col gap-3">
          ...
          <div className="flex gap-3 flex-wrap items-end">
            <div>
              <label className="block text-xs text-muted-foreground mb-1">{t('pages.rules.ruleType')}</label>
              <select
                className={selectClass}
                value={ruleType}
                onChange={e => {
              ...
              >
                {ALL_RULE_TYPES.map(rt => (
                  <option key={rt} value={rt}>{t(`ruleTypes.${rt}`)}</option>
                ))}
              </select>
            </div>
            {needsTeacher && (
              <div>
                <label className="block text-xs text-muted-foreground mb-1">{t('pages.rules.teacher')}</label>
                <select
                  className={selectClass}
                  value={teacherId ?? ''}
            ...
            {needsGroup && (
              <div>
                <label className="block text-xs text-muted-foreground mb-1">{t('pages.rules.group')}</label>
                <select
                  className={selectClass}
                  value={groupId ?? ''}
            ...
            <div>
              <label className="block text-xs text-muted-foreground mb-1">{t('pages.rules.value')}</label>
              <input
                type="number"
                min="0"
      ...
      {selectedAnnexId === null ? (
        <p className="text-sm text-muted-foreground">{t('schedule.selectAnnex')}</p>
      ) : isLoading ? (
