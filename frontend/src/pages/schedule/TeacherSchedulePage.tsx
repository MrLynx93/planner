...
export function TeacherSchedulePage() {
  ...
  const { data: annexes = [], isLoading: annexesLoading } =
    useGetAnnexesQuery();
  const { data: teachers = [], isLoading: teachersLoading } =
    useGetAnnexTeachersQuery(selectedAnnexId!, {
  ...
  const { data: exceptions = [] } = useGetExceptionsQuery(selectedAnnexId!, {
    skip: selectedAnnexId === null,
  });
  ...
  const skipMonth = (i: number) =>
    selectedAnnexId === null ||
    viewMode !== 'month' ||
    i >= monthWeekStarts.length;
  const mwStr = (i: number) =>
    monthWeekStarts[i]?.toISOString().slice(0, 10) ?? '';
  const mw0 = useGetEffectiveScheduleQuery(
    { annexId: selectedAnnexId ?? 0, weekStart: mwStr(0) },
    { skip: skipMonth(0) }
  );
  const mw1 = useGetEffectiveScheduleQuery(
    { annexId: selectedAnnexId ?? 0, weekStart: mwStr(1) },
    { skip: skipMonth(1) }
  );
  const mw2 = useGetEffectiveScheduleQuery(
    { annexId: selectedAnnexId ?? 0, weekStart: mwStr(2) },
    { skip: skipMonth(2) }
  );
  const mw3 = useGetEffectiveScheduleQuery(
    { annexId: selectedAnnexId ?? 0, weekStart: mwStr(3) },
    { skip: skipMonth(3) }
  );
  const mw4 = useGetEffectiveScheduleQuery(
    { annexId: selectedAnnexId ?? 0, weekStart: mwStr(4) },
    { skip: skipMonth(4) }
  );
  const mw5 = useGetEffectiveScheduleQuery(
    { annexId: selectedAnnexId ?? 0, weekStart: mwStr(5) },
    { skip: skipMonth(5) }
  );
  ...
  const monthBlocksByDate = useMemo(() => {
    ...
    weekData.forEach((data, i) => {
      ...
      for (const block of data) {
        if (selectedTeacherId !== null && block.teacherId !== selectedTeacherId)
          continue;
  ...
  const weekDateSet = useMemo(
    () => new Set(weekDays.map((d) => d.toISOString().slice(0, 10))),
    [weekDays]
  );
  const allBlocksByTimeBlockId = useMemo(
    () => new Map(allBlocks.map((b) => [b.timeBlockId, b])),
    [allBlocks]
  );

  const exceptionReasonByTimeBlockId = useMemo((): Map<
    number,
    ExceptionReason
  > => {
  ...
  const removedExceptions = useMemo(
    (): RemovedExceptionBlock[] =>
      exceptions.flatMap((ex) =>
        ex.modifications
          ...
          .filter(
            (m) =>
              allBlocksByTimeBlockId.get(m.timeBlockId)?.teacherId ===
              selectedTeacherId
          )
          .map((m) => ({
            id: m.id,
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {selectedAnnexId !== null && viewMode === 'week' && (
          <ViolationsPanel annexId={selectedAnnexId} weekStart={weekStart} />
        )}
      </div>
