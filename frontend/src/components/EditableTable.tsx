...
export function EditableTable<T extends { id: number | null }>({
  ...
  async function handleAdd() {
    ...
    await onAdd(
      Object.fromEntries(columns.map((c) => [c.key, newValues[c.key].trim()]))
    );
  ...
  async function handleSave(item: T) {
    ...
    await onSave(
      item,
      Object.fromEntries(columns.map((c) => [c.key, editValues[c.key].trim()]))
    );
    setEditingId(null);
  }
          ...
          {columns.map((col) => (
            <td key={col.key} className="pr-4 align-middle">
              <input
                ...
                onChange={(e) =>
                  setNewValues((v) => ({ ...v, [col.key]: e.target.value }))
                }
