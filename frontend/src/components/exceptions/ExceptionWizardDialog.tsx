...
} from './TemplateBlockSelector';
import { formatTime } from '@/components/schedule/utils';
import type {
  CreateExceptionRequest,
  CreateModificationItemRequest,
...
interface AbsenceFormProps {
  ...
  preselectedBlock?: ScheduleBlock;
  onSubmit: (
    mods: CreateModificationItemRequest[],
    title: string,
    note: string
  ) => void;
}
...
function AbsenceForm({
  ...
  const [covers, setCovers] = useState<
    Record<
      string,
      { teacherId: number | null; startTime: string; endTime: string }
    >
  >({});
  const [note, setNote] = useState('');

  const teacherBlocks = allBlocks.filter(
    (b) => b.teacherId === teacherId && b.type === 'TEMPLATE'
  );
  ...
  const toggleBlock = (block: SelectableBlock) => {
    setSelected((prev) =>
      prev.some(
        (s) => s.timeBlockId === block.timeBlockId && s.date === block.date
      )
        ? prev.filter(
            (s) =>
              !(s.timeBlockId === block.timeBlockId && s.date === block.date)
          )
        : [...prev, block]
    );
  };
      ...
      {dates.length > 0 && teacherBlocks.length > 0 ? (
        <div>
          <div className="text-xs font-medium text-muted-foreground mb-2">
            {t('exceptions.wizard.selectBlocks')}
          </div>
          <TemplateBlockSelector
          ...
          {selected.length > 0 && (
            <div className="mt-3">
              <div className="text-xs font-medium text-muted-foreground mb-2">
                {t('exceptions.wizard.assignCover')}
              </div>
                      ...
                      {cover.teacherId && (
                        <>
                          <input
                            ...
                            value={cover.startTime.slice(0, 5)}
                            onChange={(e) =>
                              setCovers((prev) => ({
                                ...prev,
                                [key]: {
        ...
        <p className="text-sm text-muted-foreground">
          {t('exceptions.wizard.noBlocksForTeacher')}
        </p>
      ) : null}

      <div className="flex flex-col gap-1">
        <label className="text-xs font-medium text-muted-foreground">
          {t('exceptions.wizard.note')}
        </label>
        <textarea
          className="rounded-md border border-border bg-background px-2.5 py-1.5 text-sm resize-none"
          rows={2}
          value={note}
...
function ExchangeForm({
  ...
  const [teacherAId, setTeacherAId] = useState<number | null>(
    preselectedBlock?.teacherId ?? teachers[0]?.teacherId ?? null
  );
  const [dateA, setDateA] = useState(today);
  const [blockAId, setBlockAId] = useState<number | null>(
    preselectedBlock?.timeBlockId ?? null
  );

  const [teacherBId, setTeacherBId] = useState<number | null>(
    teachers[1]?.teacherId ?? null
  );
  ...
  const dowA = [
    ...
    'FRIDAY',
    'SATURDAY',
  ][new Date(dateA + 'T00:00:00').getDay()];
  const dowB = [
    'SUNDAY',
    'MONDAY',
    'TUESDAY',
    ...
    'FRIDAY',
    'SATURDAY',
  ][new Date(dateB + 'T00:00:00').getDay()];

  const blocksA = allBlocks.filter(
    (b) =>
      b.teacherId === teacherAId &&
      b.type === 'TEMPLATE' &&
      b.dayOfWeek === dowA
  );
  const blocksB = allBlocks.filter(
    (b) =>
      b.teacherId === teacherBId &&
      b.type === 'TEMPLATE' &&
      b.dayOfWeek === dowB
  );

  const blockA =
    blocksA.find((b) => b.timeBlockId === blockAId) ?? blocksA[0] ?? null;
  const blockB =
    blocksB.find((b) => b.timeBlockId === blockBId) ?? blocksB[0] ?? null;
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-2 rounded-md border border-border p-3">
          <div className="text-xs font-semibold uppercase text-muted-foreground">
            {t('exceptions.wizard.teacherA')}
          </div>
          <select
            className="rounded border border-border bg-background px-2 py-1 text-sm"
            value={teacherAId ?? ''}
            onChange={(e) => {
              setTeacherAId(Number(e.target.value));
              setBlockAId(null);
            }}
          >
            {teachers.map((tc) => (
          ...
          {blocksA.length > 0 ? (
            <select
              className="rounded border border-border bg-background px-2 py-1 text-sm"
              value={blockAId ?? blocksA[0]?.timeBlockId ?? ''}
              onChange={(e) => setBlockAId(Number(e.target.value))}
            >
              {blocksA.map((b) => (
                <option key={b.timeBlockId} value={b.timeBlockId}>
                  {b.groupName} {formatTime(b.startTime)}–
            ...
            <p className="text-xs text-muted-foreground">
              {t('exceptions.wizard.noBlocksOnDay')}
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col gap-2 rounded-md border border-border p-3">
          <div className="text-xs font-semibold uppercase text-muted-foreground">
            {t('exceptions.wizard.teacherB')}
          </div>
          <select
            className="rounded border border-border bg-background px-2 py-1 text-sm"
            value={teacherBId ?? ''}
            onChange={(e) => {
              setTeacherBId(Number(e.target.value));
              setBlockBId(null);
            }}
          >
            {teachers.map((tc) => (
          ...
          {blocksB.length > 0 ? (
            <select
              className="rounded border border-border bg-background px-2 py-1 text-sm"
              value={blockBId ?? blocksB[0]?.timeBlockId ?? ''}
              onChange={(e) => setBlockBId(Number(e.target.value))}
            >
              {blocksB.map((b) => (
                <option key={b.timeBlockId} value={b.timeBlockId}>
                  {b.groupName} {formatTime(b.startTime)}–
...
function OvertimeForm({ teachers, allBlocks, onSubmit }: OvertimeFormProps) {
  ...
  const groups = Array.from(
    new Map(
      allBlocks.map((b) => [b.groupId, { id: b.groupId, name: b.groupName }])
    ).values()
  );
...
function ScheduleAdjustmentForm({
  teachers,
  allBlocks,
  preselectedBlock,
  onSubmit,
}: ScheduleAdjustmentFormProps) {
  const { t } = useTranslation();
  const today = new Date().toISOString().slice(0, 10);
      ...
      {dates.length > 0 && (
        <>
          ...
          {selected.length > 0 && (
            <div>
              <div className="text-xs font-medium text-muted-foreground mb-2">
                {t('exceptions.wizard.newTimes')}
              </div>
              <div className="flex flex-col gap-2">
                {selected.map((b) => {
                  const k = key(b);
                  const times = newTimes[k] ?? {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-muted-foreground">
                        {t('exceptions.wizard.originalTime')}:{' '}
                        {formatTime(b.startTime)}–{formatTime(b.endTime)}
                      </span>
                      <span className="ml-auto flex items-center gap-1">
                        <input
                          ...
                          onChange={(e) =>
                            setNewTimes((prev) => ({
                              ...prev,
                              [k]: { ...times, startTime: e.target.value },
                            }))
                          }
                        />
                        <span>–</span>
                        <input
                          ...
                          onChange={(e) =>
                            setNewTimes((prev) => ({
                              ...prev,
                              [k]: { ...times, endTime: e.target.value },
                            }))
                          }
                        />
                      </span>
...
export function ExceptionWizardDialog({
  open,
  onClose,
  onSubmit,
  teachers,
