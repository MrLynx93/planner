      ...
      <div className="m-16 min-w-[560px] border border-gray-400 rounded">
        ...
        <div className="flex" style={{ height: gridHeight }}>
          ...
          {WEEK_DAYS.map((day) => {
            ...
            const dayGhosts = showExceptions
              ? (removedExceptions?.filter((re) => re.dayOfWeek === day) ?? [])
              : [];
            const GHOST_ID_OFFSET = 1_000_000;
            const allLayoutBlocks = [
              ...dayBlocks,
              ...dayGhosts.map((re) => ({
                id: re.id + GHOST_ID_OFFSET,
            ...
            return (
              <div
                key={day}
                className="flex-1 relative border-l border-gray-500"
              >
                {/* Hour lines */}
                {hours.map((h) => (
                  <div
                ...
                {dayBlocks.map((block) => {
                  ...
                  const exceptionReason =
                    showExceptions && block.type === 'MODIFICATION'
                ...
                {dayGhosts.map((re) => {
                  ...
                  const { columnIndex, columnCount } = columns.get(
                    re.id + GHOST_ID_OFFSET
                  )!;
                    ...
                    >
                      <div className="relative z-20 px-1.5 py-0.5 overflow-hidden h-full">
                        ...
                        >
                          {colorBy === 'group'
                            ? re.groupName
                            : `${re.teacherFirstName} ${re.teacherLastName}`}
                        </p>
                        {height >= 32 && (
                          <p
                            className="text-xs leading-tight truncate opacity-70"
                            style={{ color: '#ef4444' }}
                          >
                        ...
                        >
                          {t('schedule.removed')} ·{' '}
                          {t(`exceptions.reasons.${re.reason}`)}
                        </p>
                      </div>
                    </div>
                  );
